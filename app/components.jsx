...
function PawTopBar({ variant = 'home', title, sub, onBack, right, left, dark }) {
  const wrap = {
    ...
    padding: '8px 6px',
    height: 52,
    background: variant === 'transparent' ? 'transparent' : 'var(--background-elevated-normal)',
    borderBottom: variant === 'transparent' ? 'none' : '1px solid var(--line-normal-neutral)',
  ...
  if (variant === 'home') {
    return (
      <div style={wrap}>
        ...
        }}>몽냥<span style={{color:'var(--primary-normal)',marginLeft:1}}>.</span></span>
        <div style={{ flex: 1 }} />
        {right}
      </div>
    );
  ...
  return (
    <div style={wrap}>
      ...
      <div style={{ flex: 1, textAlign: 'center', overflow: 'hidden' }}>
        {title && <div style={{ font: '700 16px/1.3 var(--font-sans)', letterSpacing: '-0.012em', color: 'var(--label-strong)', textOverflow:'ellipsis', overflow:'hidden', whiteSpace:'nowrap' }}>{title}</div>}
        {sub && <div style={{ font: '500 11px/1.2 var(--font-sans)', color: 'var(--label-alternative)', marginTop: 2 }}>{sub}</div>}
      </div>
...
function PawTabBar({ active = 'home', onChange, onUpload }) {
  const tabs = [
    ...
    { id: 'discover', icon: 'search',  iconOn: 'search-thick', label: '발견' },
    { id: 'upload',   icon: 'plus',    iconOn: 'plus',         label: '업로드', center: true },
    { id: 'noti',     icon: 'bell',    iconOn: 'bell-fill',    label: '알림', badge: 5 },
    { id: 'me',       icon: 'person',  iconOn: 'person-fill',  label: '마이' },
  ];
  return (
    <div style={{
        ...
        return (
          <button key={t.id} onClick={() => onChange?.(t.id)} style={{
            ...
            <span style={{ position: 'relative' }}>
              <PawIcon name={on ? t.iconOn : t.icon} size={24} />
              {t.badge ? (
                <span style={{
...
function PawMasonry({ items, gap = 6, onTap, onLike, likedSet }) {
    ...
    <div style={{ display: 'flex', gap, padding: `0 ${gap}px ${gap}px` }}>
      {cols.map((col, ci) => (
        <div key={ci} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap }}>
          {col.map(p => (
            <PhotoTile key={p.id} p={p} onTap={onTap} onLike={onLike} liked={likedSet?.has(p.id)} />
          ))}
        </div>
      ))}
    </div>
...
function PhotoTile({ p, onTap, onLike, liked }) {

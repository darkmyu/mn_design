  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px 20px' }}>
        <p style={{ margin: '6px 0 18px', font:'500 13px/1.5 var(--font-sans)', color:'var(--label-alternative)' }}>
          등록하면 해당 친구의 <b style={{color:'var(--label-strong)'}}>전용 성장 타임라인</b>이 자동으로 시작돼요
        </p>
            ...
            {speciesOpts.map(o => {
              ...
              return (
                <button key={o.id} onClick={() => setSpecies(o.id)} style={{
                  ...
                  <span style={{ font: `${on ? 700 : 600} 12px/1 var(--font-sans)`, color: on ? PawColors.brandInk : 'var(--label-normal)' }}>{o.name}</span>
                </button>
              );
            })}
          </div>
        ...
        <Field label="품종" right={<span style={{ font:'500 11px/1 var(--font-sans)', color:'var(--label-assistive)' }}>몰라도 OK</span>}>
          <button style={{
            ...inputStyle(), display:'flex', alignItems:'center', justifyContent:'space-between',
            background:'#fff', cursor:'pointer'
          }}>
            <span style={{ color: 'var(--label-strong)', font: '600 14px/1 var(--font-sans)' }}>{breed}</span>
            <PawIcon name="chevron-down" size={16} color="var(--label-alternative)" />
          </button>
        </Field>
        ...
        <Field label="생일">
          <div style={{ display: 'flex', gap: 10 }}>
            <button style={{
              ...
              <PawIcon name="calendar" size={16} color="var(--label-alternative)" />
              <span style={{ color: 'var(--label-strong)', font: '600 14px/1 var(--font-sans)' }}>{bday}</span>
            </button>
            <button style={{
              padding: '0 14px', height: 48, borderRadius: 12, cursor:'pointer',
              background: 'var(--color-atomic-coolNeutral-98)', border:'none',
              font: '600 13px/1 var(--font-sans)', color:'var(--label-alternative)',
            }}>입양일</button>
          </div>
        </Field>
            ...
            {[
              ...
              return (
                <button key={g.id} onClick={() => setGender(g.id)} style={{
                  ...
                  background: on ? 'var(--label-strong)' : 'var(--color-atomic-coolNeutral-98)',
                  color: on ? '#fff' : 'var(--label-normal)',
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 6 }}>
        <label style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em' }}>{label}</label>
        {right}
      </div>
      {children}
    </div>
...
function inputStyle() {
  return {
    ...
    background: 'var(--color-atomic-coolNeutral-98)',
    border: 'none', borderRadius: 12,
    font: '600 14px/1 var(--font-sans)', color: 'var(--label-strong)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '8px 20px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <h1 style={{ margin: 0, font: '800 22px/1.2 var(--font-sans)', letterSpacing: '-0.024em', color: 'var(--label-strong)' }}>{pet.name}</h1>
              <PawChip tone="brandSoft" size="sm">{pet.breed}</PawChip>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 6, font: '500 12px/1 var(--font-sans)', color: 'var(--label-alternative)' }}>
              <span>{pet.age}</span><span>·</span>
              <span>입양 {pet.born}</span>
            </div>
            <div style={{ display: 'flex', gap: 14, marginTop: 10 }}>
              <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)' }}>251 <span style={{ font: '500 11px/1', color: 'var(--label-alternative)' }}>장</span></span>
              <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)' }}>12 <span style={{ font: '500 11px/1', color: 'var(--label-alternative)' }}>개월</span></span>
              <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)' }}>3 <span style={{ font: '500 11px/1', color: 'var(--label-alternative)' }}>마일스톤</span></span>
            </div>
          </div>
        </div>
          ...
          {[{id:'timeline',l:'타임라인',i:'clock'},{id:'grid',l:'사진 격자',i:'menu'}].map(m => {
            ...
            return (
              <button key={m.id} onClick={() => setMode(m.id)} style={{
                ...
                background: on ? 'var(--label-strong)' : 'var(--color-atomic-coolNeutral-98)',
                color: on ? '#fff' : 'var(--label-alternative)',
                border: 'none', cursor: 'pointer',
                font: `${on ? 700 : 600} 13px/1 var(--font-sans)`,
        ...
        <div style={{ padding: '0 16px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <div>
            <div style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-assistive)', letterSpacing: '0.04em' }}>2024 — 2025</div>
            <div style={{ font: '800 20px/1.2 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.018em', marginTop: 4 }}>몽이의 첫 1년</div>
          </div>
...
function TimelineRow({ m, i, last }) {
  const milestoneColor = m.milestone ? PawColors.brand : 'var(--line-normal-normal)';
  return (
    <div style={{ display: 'flex', gap: 12 }}>
      ...
      <div style={{ width: 32, position: 'relative', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {!last && <div style={{ flex: 1, width: 2, background: 'var(--line-normal-neutral)', marginTop: -2 }} />}
      </div>
      {/* card */}
      <div style={{ flex: 1, paddingBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 6 }}>
          <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em' }}>{m.ym}</span>
          <span style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-alternative)' }}>{m.label}</span>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ font: '500 12px/1.3 var(--font-sans)', color: 'var(--label-alternative)' }}>
              <b style={{color:'var(--label-strong)'}}>{m.count}</b>장 · {m.milestone || '평범한 한 달'}
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '18px 20px 8px' }}>
          <h3 style={{ margin: 0, font: '800 17px/1.3 var(--font-sans)', letterSpacing:'-0.018em', color:'var(--label-strong)' }}>이번 달의 이야기</h3>
          <p style={{ margin: '8px 0 0', font: '500 14px/1.6 var(--font-sans)', color: 'var(--label-normal)' }}>
        ...
        <div style={{ display:'flex', gap:8, padding:'12px 20px 6px' }}>
          {[
            ...
            <div key={s.l} style={{ flex:1, padding:'12px 12px', background:'var(--color-atomic-coolNeutral-98)', borderRadius:12 }}>
              <PawIcon name={s.i} size={16} color={PawColors.brand} />
              <div style={{ font:'800 18px/1.2 var(--font-sans)', color:'var(--label-strong)', marginTop:4, letterSpacing:'-0.018em' }}>{s.v}</div>
              <div style={{ font:'500 11px/1 var(--font-sans)', color:'var(--label-alternative)', marginTop:3 }}>{s.l}</div>
            </div>
          ))}
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#FAF6F1' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 20px' }}>
        <div style={{ textAlign: 'center', padding: '12px 0 18px' }}>
          ...
          <h1 style={{ margin: '12px 0 4px', font: '800 22px/1.2 var(--font-sans)', letterSpacing:'-0.024em', color:'var(--label-strong)' }}>
            몽이의 1년 전 오늘
          </h1>
          <p style={{ margin: 0, font: '500 13px/1.5 var(--font-sans)', color: 'var(--label-alternative)' }}>
          ...
          {[
            ...
            <div key={i} style={{
              ...
              <div style={{ padding: '10px 12px 14px' }}>
                <div style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-alternative)', letterSpacing: '0.02em' }}>{c.date}</div>
                <div style={{ font: '700 14px/1.3 var(--font-sans)', color: 'var(--label-strong)', marginTop: 4, letterSpacing: '-0.012em' }}>{c.sub}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <h4 style={{ margin: 0, font: '800 15px/1.3 var(--font-sans)', color:'var(--label-strong)', letterSpacing:'-0.012em' }}>그동안의 변화</h4>
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              <div key={r.l} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr auto', gap: 8, alignItems: 'center' }}>
                <div style={{ font: '600 12px/1 var(--font-sans)', color: 'var(--label-alternative)' }}>{r.l}</div>
                <div style={{ font: '500 13px/1 var(--font-sans)', color: 'var(--label-assistive)', textDecoration: 'line-through' }}>{r.f}</div>
                <div style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)' }}>{r.t}</div>
                <div style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px 20px' }}>
        ...
        <div style={{ marginTop: 22, display: 'flex', gap: 10 }}>
          <PawButton full size="lg" style={{ background: '#fff', color: '#2A1810' }}>
            전체 회고 보기
          </PawButton>
        </div>
        <button style={{

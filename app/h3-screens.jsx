...
function H3_FeedWithFab() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: PawColors.bg, position: 'relative' }}>
      ...
      {/* Filter chips row */}
      <div style={{ display: 'flex', gap: 6, padding: '10px 14px', background: '#fff', borderBottom:'1px solid var(--line-normal-neutral)' }}>
        {[{l:'팔로잉',on:true},{l:'발견'},{l:'내 동네'},{l:'친구 추천'}].map(c => (
          <button key={c.l} style={{
            height: 30, padding: '0 12px', borderRadius: 999,
            background: c.on ? 'var(--label-strong)' : 'var(--color-atomic-coolNeutral-98)',
            color: c.on ? '#fff' : 'var(--label-alternative)',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {/* Stories row */}
        <div style={{ display: 'flex', gap: 12, padding: '12px 14px 8px', overflowX: 'auto', background: '#fff' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6, flexShrink:0 }}>
      <div style={{
        ...
        <div style={{
          padding: 1.5, background: '#fff', borderRadius: 999,
        }}>
          <div style={{
            width: 56, height: 56, borderRadius: 999,
            background: color,
      ...
      <span style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-alternative)' }}>{name}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 16px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              padding: '4px 10px', borderRadius: 8, background: '#FF6B3D', color: '#fff',
          ...
          <div style={{
            ...
            <span style={{
              width:6, height:6, borderRadius:999, background:'#FF6B3D',
        ...
        {/* Detected list */}
        <h3 style={{ margin: 0, font: '800 17px/1.3 var(--font-sans)', letterSpacing:'-0.018em', color:'var(--label-strong)' }}>
          이 친구를 인식했어요
        </h3>
        <p style={{ margin: '6px 0 14px', font: '500 13px/1.5 var(--font-sans)', color: 'var(--label-alternative)' }}>
        ...
        <div style={{ marginTop: 14, padding: '12px 14px', background: 'var(--color-atomic-coolNeutral-98)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{
            width: 16, height: 16, borderRadius: 999,
            border: '2px solid var(--color-atomic-coolNeutral-90)',
          ...
          <span style={{ font: '500 12px/1.4 var(--font-sans)', color: 'var(--label-alternative)' }}>
            나머지 2장 분석 중... <b style={{color:'var(--label-strong)'}}>약 3초</b>
          </span>
        </div>
      </div>

      <div style={{ padding: '12px 16px 18px', borderTop: '1px solid var(--line-normal-neutral)', display: 'flex', gap: 8 }}>
...
function DetectRow({ species, name, sub, pct, primary }) {
  return (
    <div style={{
      ...
      padding: '12px 14px',
      background: primary ? PawColors.brandSoft : 'var(--color-atomic-coolNeutral-98)',
      borderRadius: 12,
    }}>
      <span style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ font: '700 14px/1.2 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em' }}>{name}</div>
        <div style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-alternative)', marginTop: 3 }}>{sub}</div>
      </div>
      <div style={{
        font: '700 12px/1 var(--font-sans)',
        color: primary ? PawColors.brand : 'var(--label-alternative)',
        padding: '5px 10px',
        background: primary ? '#fff' : 'transparent',
        borderRadius: 999,
      }}>{pct}%</div>
      <PawIcon name={primary ? 'circle-check-fill' : 'plus'} size={20} color={primary ? PawColors.brand : 'var(--label-alternative)'} />
    </div>
  );
}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 16px' }}>
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 8 }}>
            <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em' }}>어떤 친구</span>
              ...
              return (
                <button key={p.id} onClick={() => setActivePet(p.id)} style={{
                  ...
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ font: `${on ? 700 : 600} 13px/1.2 var(--font-sans)`, color: on ? PawColors.brandInk : 'var(--label-strong)', letterSpacing: '-0.012em' }}>{p.name}</div>
                    <div style={{ font: '500 10px/1 var(--font-sans)', color: 'var(--label-assistive)', marginTop: 3 }}>{p.breed}</div>
                  </div>
                </button>
              );
            })}
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 8 }}>
            <span style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em' }}>태그</span>
            <PawChip tone="brandSoft" size="sm" leadingIcon="sparkle-fill">AI 자동</PawChip>
            <span style={{ flex: 1 }} />
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {tags.map((t, i) => (
              <span key={i} style={{
                ...
                background: t.tone === 'brand' ? PawColors.brand : t.tone === 'brandSoft' ? PawColors.brandSoft : 'var(--color-atomic-coolNeutral-98)',
                color: t.tone === 'brand' ? '#fff' : t.tone === 'brandSoft' ? PawColors.brand : 'var(--label-normal)',
                borderRadius: 999,
                font: '700 12px/1 var(--font-sans)',
              }}>
                #{t.l}
        ...
        {/* Caption */}
        <div style={{ marginTop: 16 }}>
          <div style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em', marginBottom: 8 }}>캡션</div>
          <textarea
            value={caption}
          ...
          />
          <div style={{ display: 'flex', justifyContent: 'flex-end', font: '500 11px/1 var(--font-sans)', color: 'var(--label-assistive)', marginTop: 4 }}>
            {caption.length} / 500
          </div>
        </div>
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ font: '700 13px/1 var(--font-sans)', color: 'var(--label-strong)', letterSpacing: '-0.012em', marginBottom: 8 }}>공개 범위</div>
        ...
        <div style={{ display: 'flex', gap: 12, padding: '12px 14px 8px', overflowX: 'auto', background: '#fff' }}>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6, flexShrink:0 }}>
            <div style={{
              ...
              <div style={{ padding: 1.5, background: '#fff', borderRadius: 999 }}>
                <div style={{
        ...
        <div style={{ padding: '14px 12px 0' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 6px 10px' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ font: '700 13px/1.2 var(--font-sans)', color: 'var(--label-strong)' }}>몽이 · 한강공원</div>
                <div style={{ font: '500 11px/1 var(--font-sans)', color: 'var(--label-alternative)', marginTop: 3 }}>방금 · 시바 · 강아지</div>
              </div>
              <PawIcon name="more-horizontal" size={20} color="var(--label-alternative)" />
            </div>
            ...
            <div style={{ padding: '0 6px 4px', font: '500 12px/1.4 var(--font-sans)', color: 'var(--label-normal)' }}>
              <b style={{color:'var(--label-strong)'}}>몽이아빠</b> 주말 한강공원 산책! 첫 잔디 데뷔 🐾
            </div>
          </div>
        </div>

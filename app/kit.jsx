...
const PawColors = {
  bg:        'var(--background-normal-normal)',
  surface:   'var(--background-elevated-normal)',
  surfaceAlt:'var(--background-elevated-alternative)',
  line:      'var(--line-normal-normal)',
  lineSoft:  'var(--line-normal-neutral)',
  label:     'var(--label-normal)',
  labelStrong:'var(--label-strong)',
  labelAlt:  'var(--label-alternative)',
  labelHint: 'var(--label-assistive)',
  labelDis:  'var(--label-disable)',
  primary:   'var(--primary-normal)',
  primaryHover: 'var(--primary-hover)',
  white:     'var(--static-white)',
  black:     'var(--static-black)',
  // pawlog brand accent — friendlier than pure Wanted blue
  brand:     '#FF6B3D',          // warm coral (paw-print friendly)
  brandSoft: '#FFF1EB',
  brandInk:  '#7A2B0F',
};
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? 'var(--interaction-disable)' : v.bg,
...
function PawChip({ tone = 'neutral', selected, leadingDot, leadingIcon, children, onClick, size = 'md' }) {
  ...
  const sz = sizes[size];
  const tones = {
    neutral:  { bg: 'var(--color-atomic-coolNeutral-98)',  fg: PawColors.label, border: 'none' },
...
function PawAvatar({ src, name = '?', size = 40, square, ring, ringColor }) {
  ...
  const box = (
    src ? (
      <div style={{ width: size, height: size, borderRadius: square ? 12 : 999, overflow: 'hidden', background: 'var(--color-atomic-coolNeutral-97)', flexShrink: 0 }}>
  ...
  if (ring) {
    return (
      <div style={{
        ...
        <div style={{ padding: 1.5, background: '#fff', borderRadius: square ? 12 : 999, display: 'inline-flex' }}>
          {box}
        </div>
      </div>
    );
...
function PetGlyph({ pet, size = 44, ring }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: 1.5, background: '#fff', borderRadius: 999, display: 'inline-flex' }}>
        {wrap}
      </div>
    </div>
  );
